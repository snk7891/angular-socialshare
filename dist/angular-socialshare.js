...
(function withAngular(angular) {
  ...
  var directiveName = 'socialshare'
    ...
    , qzoneShare = function qzoneShare($window, attrs) {
      var urlString = 'http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=' + encodeURIComponent(attrs.socialshareUrl || $window.location.href);
      ...
      $window.open(
        urlString
        , 'QZone', 'toolbar=0,status=0,resizable=yes,width=' + attrs.socialsharePopupWidth + ',height=' + attrs.socialsharePopupHeight
        + ',top=' + ($window.innerHeight - attrs.socialsharePopupHeight) / 2 + ',left=' + ($window.innerWidth - attrs.socialsharePopupWidth) / 2);
    }
    , lineShare = function lineShare($window, attrs) {
      var params = encodeURIComponent(attrs.socialshareUrl || $window.location.href);
      if (attrs.socialshareText) {
        params = encodeURIComponent(attrs.socialshareText) + '%0d%0a' + params;
      }
      var urlString = 'http://line.me/R/msg/text/?' + params;
    ...
    , socialshareService = /*@ngInject*/  ['$window', '$log', function socialshareService($window, $log) {
      ...
      this.share = function shareTrigger(serviceShareConf) {
        ...
        switch (serviceShareConf.provider) {
          ...
          case 'qzone': {
            this.qzoneShare($window, serviceShareConf.attrs);
            break;
          }
          case 'line': {
